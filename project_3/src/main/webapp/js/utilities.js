...
function handleAlphanumericInput(inputElement, errorElementId, maxLength) {
    const errorMessage = document.getElementById(errorElementId);
    const currentValue = inputElement.value;

    // Check for invalid characters (anything that's not alphanumeric)
    if (/[^a-zA-Z0-9]/.test(currentValue)) {
        errorMessage.textContent = 'Only alphanumeric characters are allowed and special character not allowed';
        inputElement.value = currentValue.replace(/[^a-zA-Z0-9]/g, '');
    } else {
        if (currentValue.length > maxLength) {
            errorMessage.textContent = `Only ${maxLength} characters are allowed.`;
            inputElement.value = currentValue.slice(0, maxLength);
 ...
 *            input - The input field to validate.
 * @param {string}
 *            errorElementId - The ID of the error message element.
 */
